...
const ShopLeft = (props) => {
  ...
  useEffect(() => {
    ...
    if (params.category &&subCategories) {
      if(params.category!==props.searchTerm)
      {
        props.setSearchTerm('')
        const matchingOption =subCategories.find(option => option.name.toLowerCase() === params.category.toLowerCase());
        if (matchingOption) {
